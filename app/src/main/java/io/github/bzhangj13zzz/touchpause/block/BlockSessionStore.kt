...
class BlockSessionStore(context: Context) {
    ...
    /** Input capture cannot survive a reboot, so advisory ownership must not survive one either. */
    private fun reconcileCurrentBoot() = synchronized(stateLock) {
        val currentBoot = runCatching {
            Settings.Global.getInt(appContext.contentResolver, Settings.Global.BOOT_COUNT)
        }.getOrNull() ?: return@synchronized
        val storedBoot = preferences.getInt(KEY_BOOT_COUNT, UNKNOWN_BOOT)

        if (storedBoot != UNKNOWN_BOOT && storedBoot != currentBoot) {
            preferences.edit().clear().putInt(KEY_BOOT_COUNT, currentBoot).commit()
            processPendingRootToken = null
        } else if (storedBoot == UNKNOWN_BOOT) {
            // Preserve an in-flight session when upgrading from a build without this marker.
            preferences.edit().putInt(KEY_BOOT_COUNT, currentBoot).commit()
        }
    }
